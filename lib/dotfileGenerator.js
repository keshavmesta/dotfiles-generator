...
module.exports = function (files, options) {
  ...
  files.forEach(function (filename) {
    ...
    if (!this.overwrite && !this.merge && isDestinationExist) {
      logger(
        filename +
        chalk.yellow(' -> ') +
        destination +
    ...
    if(this.merge && isDestinationExist) {
      // Merge files
      let status = merge(source, destination);
      let message, type;
      ...
      if(status) {
        ...
        message =  ' [Not merged. Please check for any syntax errors.]';
        type = ERROR;
      }
